...
//            "2027-04-${currentYear+4}", "2028-04-${currentYear+5}")
    ...
    {
        /*val dataRefer = FirebaseFirestore.getInstance().collection(Constants.GARBAGE).document(FireStoreClass().getCurrentUserId()
        dataRefer.get().addOnSuccessListener {document->

            val generalWaste = document.get("amount").toString().toFloat()
        ...
        var generalWaste: Float=0.0f
        var plasticWaste: Float=0.0f
        val dataRefer=FirebaseFirestore.getInstance().collection(Constants.GARBAGE)
        dataRefer.whereEqualTo("userId",FireStoreClass().getCurrentUserId()).get().addOnSuccessListener { documents ->
            for (document in documents)
            {
                if (document["type"].toString() == "plasticWaste") {
                     plasticWaste+=document["amount"].toString().toFloat()
                } else {
                    if (document["type"].toString() == "generalWaste") {
                         generalWaste+=document["amount"].toString().toFloat()
                    }
                }

            }
            //Toast.makeText(applicationContext, generalWaste.toString(), Toast.LENGTH_SHORT).show()
            val pieChart: PieChart = findViewById(R.id.activity_main_pieChart)
            DrawPieChart.setupPieChart(pieChart)
            DrawPieChart.loadPieChartData(pieChart, generalWaste,plasticWaste)
        } }
    ...
    {
        val mFireStore = FirebaseFirestore.getInstance()
        mFireStore.collection(Constants.GARBAGE).document(FireStoreClass().getCurrentUserId()).get().addOnSuccessListener { document->
